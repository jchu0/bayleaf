...
Env knobs mirror the other agent seams:
  BAYLEAF_NODE_AUTHOR_AGENT   "stub" (default, offline, $0) | "claude" (live API)
  BAYLEAF_NODE_AUTHOR_MODEL   default "claude-sonnet-5" — the mid tier (ADR-0012). Authoring one
...
class ClaudeNodeAuthor:
    ...
    ``BAYLEAF_NODE_AUTHOR_AGENT=claude``. Design guarantees mirror the other agent seams so it is
    ...
    def __init__(
        ...
        # 2048, not 1024: the proposal carries two free-text fields (summary + rationale). A rich
        # request/tool-card set can push a grounded proposal past 1024, and a truncated response
        # yields unterminated JSON that would silently degrade to the stub (mirrors the QC-triage
        # `ask` budget, 2026-07-13).
        self.model = model or os.environ.get(
            "BAYLEAF_NODE_AUTHOR_MODEL", _DEFAULT_NODE_AUTHOR_MODEL
        )
...
def get_node_author_agent() -> NodeAuthorAgent:
    ...
    Set ``BAYLEAF_NODE_AUTHOR_AGENT=claude`` to use the live agent (requires `anthropic` +
    credentials). This is the single line that flips node authoring from offline to live.
    """
    choice = os.environ.get("BAYLEAF_NODE_AUTHOR_AGENT", "stub").strip().lower()
    if choice == "claude":
