...
Why a separate module (not more constants in :mod:`~bayleaf.synthetic.generator`):
...
def build_scale_spec(
    ...
    *,
    ...
    existing :func:`~bayleaf.synthetic.generator.generate_run` can render unchanged.
...
# single command (``python -m bayleaf.synthetic``) regenerates all of them and the
...
def generate_committed(out_dir: str | Path) -> list[Path]:
    ...
    This is what the bare ``python -m bayleaf.synthetic`` invocation runs: it
