...
tagged both in-band (a ``bayleaf-synthetic`` note in ``pipeline.log``) and out-of-band
(an ``origin`` marker file), plus in ``data/README.md`` per ``docs/data/strategy.md`` —
never misrepresented as real GIAB.

The :mod:`~bayleaf.synthetic.scale` module drives the generator at volume: one large
