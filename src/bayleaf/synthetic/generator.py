...
2. **Sample IDs must not be substrings of one another** — the log rules match on
   ``sid in line``, so ``S1`` must not match a line about ``S10``. The two committed
   demo runs stay in ``S1``..``S9``; larger runs zero-pad to a uniform width
   (``S01``..``S30``) in :mod:`bayleaf.synthetic.scale`, so no ID is a substring of
   another regardless of sample count.
3. **Origin is always ``contrived``** (invented values, never real GIAB data), tagged
   both in-band (a ``bayleaf-synthetic`` note in ``pipeline.log``) and out-of-band
   (an ``origin`` marker file). Both derive from :data:`ORIGIN_LABEL` so they cannot
   drift apart.
...
# The origin label every generated run self-declares, in the data-strategy taxonomy
# (docs/data/strategy.md): ``contrived`` = invented values with realistic formats,
# *not* derived from real data. Deliberately NOT ``synthetic`` — the strategy reserves
# that for data perturbed *from* a real sample, which this generator does not do. Both
# the out-of-band ``origin`` marker file and the in-band ``pipeline.log`` tag derive
# from this one constant so the two can never disagree (the bug this replaced was a
# hand-added marker reading ``synthetic`` while the log tag read ``contrived``).
ORIGIN_LABEL = "contrived"
...
def _render_log(spec: RunSpec) -> str:
    ...
    and never trip a rule. The ``bayleaf-synthetic`` line is the in-band origin tag.
    """
    ...
    lines = [
        ...
        f"{ts(1)} [INFO] bayleaf-synthetic: {ORIGIN_LABEL.upper()} demo run generated by "
        f"bayleaf.synthetic.generator (origin={ORIGIN_LABEL}); "
        "invented values, not real GIAB data",
...
def generate_run(spec: RunSpec, out_dir: str | Path) -> Path:
    ...
    Creates ``out_dir/<run_id>/`` with the five artifacts the parsers read plus an
    ``origin`` marker file (:data:`ORIGIN_LABEL`). The output is deterministic (same
    spec -> byte-identical files), so committed demo runs can be regenerated and
    diffed. Returns the created run directory.
    """
    ...
    (run_dir / "origin").write_text(f"{ORIGIN_LABEL}\n", encoding="utf-8", newline="\n")
    return run_dir
...
# bayleaf.synthetic.generator` reproduces exactly what's committed under data/.
...
def main(argv: list[str] | None = None) -> None:
    ...
    Usage: ``python -m bayleaf.synthetic.generator [OUT_DIR]`` (defaults to the
