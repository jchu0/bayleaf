...
A boundary port sibling to :mod:`bayleaf.notify` and :mod:`bayleaf.persistence`, sitting
...
decide (ADR-0001); swapping a store changes only *where the bytes come from*.
  ...
  * :class:`S3ArtifactStore` is OFF by default: it lazy-imports ``boto3`` (an optional
    ``[s3]`` extra) and only pulls from a bucket when ``BAYLEAF_S3_LIVE`` is armed;
    otherwise — or on ANY error — it degrades to the local store and touches no network.

:func:`get_artifact_store` flips the adapter from ``BAYLEAF_ARTIFACT_STORE=local|s3``
(default ``local``), mirroring :func:`bayleaf.notify.get_notifier`. :func:`run_gate_from_store`
...
# Env knob selecting the adapter (mirrors BAYLEAF_NOTIFIER); nothing hardcoded — see
# .env.example. Default is the offline local store so the demo/tests never reach a network.
_ENV_ARTIFACT_STORE = "BAYLEAF_ARTIFACT_STORE"


def get_artifact_store() -> ArtifactStore:
    """Select the artifact store from the environment (default: the offline local store).

    Set ``BAYLEAF_ARTIFACT_STORE=s3`` to use :class:`S3ArtifactStore` — whose live pull still
    stays guarded behind ``BAYLEAF_S3_LIVE`` and degrades to local when unarmed or on error.
