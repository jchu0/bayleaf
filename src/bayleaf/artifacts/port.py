...
port — a sibling of :mod:`bayleaf.notify` and :mod:`bayleaf.persistence`, with no
...
:func:`bayleaf.parsers.load_run` reads them. It never inspects, transforms, or
...
for :class:`~bayleaf.artifacts.local.LocalArtifactStore`, a run id resolving to
``s3://bucket/prefix/<run_id>/`` for :class:`~bayleaf.artifacts.s3.S3ArtifactStore`),
...
class ArtifactStore(Protocol):
    ...
    it*. An adapter is injected at the edge (via :func:`bayleaf.artifacts.get_artifact_store`
    or directly) and must satisfy two guarantees so it stays safe and off the critical path:

      1. :meth:`fetch` returns a local directory that :func:`bayleaf.parsers.load_run` can
