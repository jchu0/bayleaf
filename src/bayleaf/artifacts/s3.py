...
local temp directory, then the unchanged :func:`bayleaf.parsers.load_run` reads them — so the
deterministic gate is identical whether a run lives on disk or in a bucket. Its safety shape
deliberately mirrors :class:`bayleaf.notify.SlackNotifier` so the seam is safe to flip on:

  * ``boto3`` is imported LAZILY (it is an optional ``[s3]`` extra, not a core dep), so the
    package installs and runs without it.
  * The live pull is opt-in via ``BAYLEAF_S3_LIVE`` (:func:`_s3_live_enabled`). Pulling real
  ...
  * ANY error — absent ``boto3``, absent/invalid creds, an API failure, an unconfigured bucket —
    degrades to the offline :class:`~bayleaf.artifacts.local.LocalArtifactStore`. An artifact
...
_ENV_S3_LIVE = "BAYLEAF_S3_LIVE"
_ENV_S3_BUCKET = "BAYLEAF_S3_BUCKET"
_ENV_S3_PREFIX = "BAYLEAF_S3_PREFIX"
...
def _s3_live_enabled() -> bool:
    ...
    Off unless ``BAYLEAF_S3_LIVE`` is explicitly truthy in the environment. Read from
...
class S3ArtifactStore:
    ...
    def fetch(self, run_ref: RunRef) -> Path:
        ...
        Off by default: without ``BAYLEAF_S3_LIVE`` armed (or with an unconfigured bucket)
        ...
        # --- live-pull seam (reached only when BAYLEAF_S3_LIVE is armed) -------------------
        try:
    ...
    def _download_run(self, client: Any, run_ref: str) -> Path:
        ...
        Returns the temp dir for :func:`bayleaf.parsers.load_run` to read. If the prefix has no
        ...
        dest = Path(tempfile.mkdtemp(prefix="bayleaf-s3-"))
