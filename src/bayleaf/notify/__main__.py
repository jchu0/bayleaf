...
Loads ``.env`` (so ``BAYLEAF_*`` are in the environment *before* the notifier resolves
its channel/URL/token), runs the gate, and notifies each actionable card through the
notifier chosen by ``BAYLEAF_NOTIFIER`` (``stub`` by default — $0, offline, nothing
leaves the machine; ``slack`` | ``teams`` | ``discord`` are the adapters). A live post
additionally needs that adapter's own ``*_LIVE`` flag armed (``BAYLEAF_SLACK_LIVE`` /
``BAYLEAF_TEAMS_LIVE`` / ``BAYLEAF_DISCORD_LIVE``) + its creds/URL (see
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    if not args:
        print("usage: python -m bayleaf.notify <run_dir>", file=sys.stderr)
        return 2
    run_dir = args[0]
