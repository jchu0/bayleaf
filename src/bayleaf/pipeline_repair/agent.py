...
Env knobs mirror the other agent seams:
  BAYLEAF_PIPELINE_REPAIR_AGENT   "stub" (default, offline, $0) | "claude" (live API)
  BAYLEAF_PIPELINE_REPAIR_MODEL   default "claude-opus-4-8" — the Opus-high tier, because
...
class ClaudeRepairAgent:
    ...
    Nothing here runs unless ``BAYLEAF_PIPELINE_REPAIR_AGENT=claude``. Design guarantees
...
def get_repair_agent() -> RepairAgent:
    ...
    Set ``BAYLEAF_PIPELINE_REPAIR_AGENT=claude`` to use the live agent (requires `anthropic`
    + credentials). This is the single line that flips repair from offline to live.
    """
    choice = os.environ.get("BAYLEAF_PIPELINE_REPAIR_AGENT", "stub").strip().lower()
    if choice == "claude":
