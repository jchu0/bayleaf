...
class PipelineStage(str, Enum):
    ...
    see ``source/bayleaf.dc.html``), plus ``intake`` for the accessioning / metadata step
...
class RecurringSignature(BaseModel):
    ...
    (``api.main.MonitoringSignature`` — a distinct :attr:`~bayleaf.models.Finding.signature`
...
def assemble_recurring_signatures(
    ...
    :attr:`~bayleaf.models.Finding.signature` across every served run's cards, fix the
