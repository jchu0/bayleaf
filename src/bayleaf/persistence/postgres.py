...
The production sibling of :class:`~bayleaf.persistence.sqlite.SqliteRepository`: it
implements the same :class:`~bayleaf.persistence.repository.Repository` interface over a
Postgres server, so the core still never touches a DB directly and swapping the adapter is a
single factory line (:func:`bayleaf.persistence.get_repository`). Kept behind the optional
...
4. The table-layout version lives in a tiny ``bayleaf_meta`` row (Postgres has no
...
_SCHEMA = """
CREATE TABLE IF NOT EXISTS bayleaf_meta (
...
class PostgresRepository:
    ...
    def __init__(self, dsn: str | None = None) -> None:
        ...
        # never at module import, so `import bayleaf.persistence` works without the extra.
        try:
            import psycopg
    ...
    def initialize(self) -> None:
        ...
        row = self._conn.execute(
            "SELECT value FROM bayleaf_meta WHERE key = 'persist_schema_version'"
        ).fetchone()
        ...
        self._conn.execute(
            """INSERT INTO bayleaf_meta (key, value) VALUES ('persist_schema_version', %s)
