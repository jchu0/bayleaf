...
# Env knobs selecting the adapter (mirrors BAYLEAF_ARTIFACT_STORE / BAYLEAF_NOTIFIER).
# Default is the offline SQLite store so the demo/tests never reach a network.
_ENV_REPOSITORY = "BAYLEAF_REPOSITORY"
_ENV_SQLITE_PATH = "BAYLEAF_DB_PATH"
...
def get_repository(*, sqlite_path: str | None = None) -> Repository:
    ...
    ``BAYLEAF_REPOSITORY=postgres`` uses :class:`PostgresRepository` (needs the ``[postgres]``
    ...
    if choice == "postgres":
        try:
            ...
            _log.warning(
                "BAYLEAF_REPOSITORY=postgres unavailable (%s); falling back to SQLite.",
                type(exc).__name__,
            )
