"""CLI entry point for rebuilding the relational projection from a JSONL ledger.
    ...
    python -m bayleaf.persistence.rebuild <ledger.jsonl> [<db.sqlite>]

The target adapter is chosen by :func:`bayleaf.persistence.get_repository` — SQLite by default
(a ``db`` path, or in-memory if omitted), or Postgres when ``BAYLEAF_REPOSITORY=postgres`` +
``DATABASE_URL`` are set (ADR-0016), in which case ``db`` is ignored. The reusable functions
(`rebuild_db`, `read_ledger`) live in :mod:`bayleaf.persistence.replay`; this module is a thin
...
def main(argv: list[str] | None = None) -> int:
    ...
    parser = argparse.ArgumentParser(
        prog="python -m bayleaf.persistence.rebuild",
    ...
    parser.add_argument(
        "db",
        nargs="?",
        help="SQLite projection path. Omit for Postgres (BAYLEAF_REPOSITORY=postgres + DSN).",
    )
    args = parser.parse_args(argv)

    # get_repository honours BAYLEAF_REPOSITORY (postgres|sqlite) + degrades to SQLite on any
