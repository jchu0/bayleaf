...
adds NO code to :mod:`pipeguard.engine`. It is a two-line composition: stage the run to a
local directory via the injected :class:`~pipeguard.artifacts.port.ArtifactStore`, then
hand that directory to the unchanged :func:`pipeguard.engine.run_gate_from_dir`. The store
...
def run_gate_from_store(
    ...
    A drop-in for :func:`pipeguard.engine.run_gate_from_dir` that adds a store hop in front:
