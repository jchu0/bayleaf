"""Package CLI: ``python -m pipeguard.synthetic`` — regenerate demo data, at scale.
...
``python -m pipeguard.synthetic.generator`` form emits a ``RuntimeWarning``. All
...
def _build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        prog="python -m pipeguard.synthetic",
...
def main(argv: list[str] | None = None) -> None:
    ...
    if getattr(args, "func", None) is None:
        # Bare ``python -m pipeguard.synthetic`` -> regenerate the committed runs, the
