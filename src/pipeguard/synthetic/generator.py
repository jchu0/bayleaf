...
2. **Sample IDs must not be substrings of one another** — the log rules match on
   ...
   (``S01``..``S30``) in :mod:`pipeguard.synthetic.scale`, so no ID is a substring of
   another regardless of sample count.
3. **Origin is always ``contrived``** (invented values, never real GIAB data), tagged
   both in-band (a ``pipeguard-synthetic`` note in ``pipeline.log``) and out-of-band
...
def _render_log(spec: RunSpec) -> str:
    ...
    lines = [
        ...
        f"{ts(1)} [INFO] pipeguard-synthetic: {ORIGIN_LABEL.upper()} demo run generated by "
        f"pipeguard.synthetic.generator (origin={ORIGIN_LABEL}); "
