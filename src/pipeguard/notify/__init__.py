...
(``PIPEGUARD_NOTIFIER=stub|slack|teams|discord``, default stub). Each real adapter's live
send is opt-in via its own ``*_LIVE`` flag (``PIPEGUARD_SLACK_LIVE`` / ``PIPEGUARD_TEAMS_LIVE``
/ ``PIPEGUARD_DISCORD_LIVE``, T-015b / T-035) — unarmed, it degrades to the stub.
"""
