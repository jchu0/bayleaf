...
WHY ``PIPEGUARD_DATA_ROOT`` (not a new ``PIPEGUARD_RUN_ROOT``): the read-API already resolves runs
under ``PIPEGUARD_DATA_ROOT`` at call-time (``api/card_readout.py::_data_root``). Introducing a
...
# The default when ``PIPEGUARD_DATA_ROOT`` is unset: the repo's committed ``data/`` dir.
# ``settings.py`` lives at ``src/pipeguard/`` so the repo root is three parents up.
...
RUN_ROOT_ENV = "PIPEGUARD_DATA_ROOT"


def run_store_root() -> Path:
    """Return the root directory under which run dirs are discovered/materialized.

    Resolved at call-time from ``PIPEGUARD_DATA_ROOT`` (else the repo ``data/`` default), so a test
