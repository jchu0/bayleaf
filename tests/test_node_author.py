...
def test_claude_proposal_requests_a_larger_budget_than_1024(monkeypatch):
    ...
    _claude_agent(monkeypatch, client).propose("fastp")
    assert client.create_kwargs["max_tokens"] >= 2048


def test_claude_path_falls_back_to_stub_on_truncation(monkeypatch):
    ...
    proposal = _claude_agent(monkeypatch, _RecordingClient(truncated)).propose("fastp")
