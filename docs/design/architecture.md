...
3. **Advisory agents (OFF the deterministic critical path).** The QC-triage agent (`triage/`,
   ...
   real proposal instead of a static preview. **The accept path is now backend-built too
   (2026-07-11, T-135, "W2 backend")**: `POST /api/builder/node-proposal/accept`
   (`reviewer`/`approver`) re-derives the proposal server-side, runs it through a new
   `pipeguard.node_author.conformance.check_conformance()` (mechanically enforcing
   `agent-authoring-contract.md`'s capability pins), and stores a `draft` `LibraryEntry` — metadata
   only, never a runnable command — in the new `api/library_store.py`
   (`PIPEGUARD_LIBRARY_STORE=jsonl|sqlite`, ADR-0016 item 9). A companion
   `src/pipeguard/node_author/importer.py` deterministically parses an nf-core
   `nextflow_schema.json` into a `NodeProposal` for a tool **not** in the curated corpus (the
   structured half of the originally-proposed doc-drop importer); free-text `--help`/README
   parsing stays deferred. **Still deferred: the Builder's own "Accept to library" button** — the
   two new endpoints have no frontend caller yet, so accept is reachable only over the wire
   (`curl`/tests), not from the UI. See [agent-authoring-contract.md](agent-authoring-contract.md).
   ...
   **no BaseSpace connector** and no conversational multi-turn triage chat (both still wishlist);
   ...
   - **Wave 10 (2026-07-10, commits `71d4ff9`→`6b571a4`, T-046/T-118).** Two independent pieces,
     ...
     **narrower than the original design note** ([node-authoring-agent.md](node-authoring-agent.md)):
     ...
     over a tool's own dropped docs — it cannot onboard a genuinely new tool yet. **Corrected
     2026-07-11 (branch `feat/custom-script-io`, Branch A): now 10 cards** — the unwired Truth VCF
     reference-node card was retired, see the custom-script-I/O bullet near the end of this list and
     [ADR-0020](../adr/ADR-0020-operator-authored-custom-processes.md). **(2) UIC-1..16 —
   ...
   - **Fleet: node-author W2 backend + a11y sweep + reliability tests (2026-07-11, commits
     `5a3dd6a`/`4427ec2`/`63b87ce`, T-135/T-136/T-137,
     [journal](../journal/2026-07-11-fleet.md)) — three independent pieces, no verdict/gate/
     ADR-0001 boundary changed.** **(1) Node-author accept→library (T-135)** — see the Advisory
     agents bullet above and [ADR-0016 item 9](../adr/ADR-0016-postgres-port.md); +34 tests
     (`test_library_store.py`, `test_node_author_accept_api.py`, `test_node_author_conformance.py`,
     `test_node_author_importer.py`). **(2) A11y baseline extends past `Toast`/`ConfirmDialog`
     (T-136, REQ-NF-070 addendum)** — `Tabs` gains roving-tabindex + Arrow/Home/End keyboard nav;
     `Pager` gets a `nav` landmark + `aria-current`/`aria-label` per page; `SegmentedControl` gets
     `role="radiogroup"`/`role="radio"` + `aria-checked`; `RunSelector` gets the combobox/listbox
     pattern (`aria-activedescendant` + arrow nav); Submit/Accession/Settings form inputs gain
     `htmlFor`/`id` label association, `aria-label` on grid-row inputs with visual-only headers, and
     `aria-describedby` on hints. Verdict-token contrast was **verified** to pass WCAG AA (all 8
     fg/bg pairings, 5.5–9:1) — no `index.css` change was needed. Frontend-only
     (`git diff --stat` scoped to `components/{Tabs,Pager,SegmentedControl,RunSelector}.tsx` +
     `screens/{Submit,Accession,Settings}.tsx`). **(3) Reliability test gaps closed (T-137,
     closes the EVAL-008 "not covered by an automated test" note)** — `test_job_store.py` (12→16)
     gains a LIVE `killpg` process-group reap test (a driver whose grandchild outlives the direct
     child is fully reaped on timeout) plus a negative control that stubs the group-kill down to a
     direct-child-only `proc.kill()` and proves the grandchild then survives — so the reap
     assertion is shown to actually detect an orphan, not pass vacuously; plus an atomic
     concurrent-same-run-id-submit test and the Builder-run `_reconcile → lost` branch test.
     Test-only, skip-gated to POSIX process groups + `/bin/sh`. Census: **555 tests collected / 41
     files** (was 517/37), **549 passed / 6 skipped** (verified via `uv run pytest`), ruff+mypy
     clean. **Deferred, named, not silently dropped:** the Builder's own "Accept to library"
     button (no frontend caller of the accept/library endpoints yet); the free-text `--help`/README
     half of the doc-drop importer (only the structured `nextflow_schema.json` path is built); the
     `draft→approved` library-entry transition; a roster-wide, CI-enforced
     `test_agent_conformance.py` (today's `check_conformance()` is invoked at accept time only, not
     as a parametrized sweep over every advisory agent); the Slurm executor profile stays
     CONFIG-verified, not CLUSTER-verified (REQ-F-090); a live multi-sample Nextflow run stays
     unverified (REQ-F-095, item above).
   - **Custom-script processes + a sandboxed file browser (2026-07-11, branch
     `feat/custom-script-io`, commit `6f1c758` [Branch A] + working tree [Branch B],
     [ADR-0020](../adr/ADR-0020-operator-authored-custom-processes.md),
     [journal](../journal/2026-07-11-custom-script-io.md)) — two independent pieces.** **Branch A**
     retires two dangling, unwired Builder placeholders — the `Truth VCF` reference source and the
     `NGSCheckMate` palette tile — and replaces them with a generic typed **File input** card
     (`makeUserNode` honors the operator-picked kind); `truth_vcf`/`ngscheckmate` stay in
     `ARTIFACT_KINDS` on both sides, only the two named palette nodes are gone. The node-author
     corpus drops `source_truth_vcf` — **10 cards (was 11)**. **Branch B adds operator-authored
     custom-script Nextflow processes** ([ADR-0020](../adr/ADR-0020-operator-authored-custom-processes.md)):
     `NfNode` gains optional `script`/`container`/`conda`; a non-empty `script` renders verbatim as a
     real, honestly-labelled process (`label 'operator_authored'`) wired from its own typed ports —
     the catalog is never consulted, even on a tool-name collision; a blank script is a
     `CompileError`, never fabricated. Threaded additively through `POST /api/pipelines/compile` and
     the pre-existing W1 `POST /api/pipelines/run` approval gate (a custom script reaches a compute
     host only inside a saved, APPROVED pipeline — the four-way safety pin in ADR-0020). The Builder
     gets a new amber-toned "Custom script" palette card + inspector
     (`CustomScriptInspector`/`BuilderModals.tsx`) and `NextflowExportModal` gains a per-file bundle
     picker. **Also new: a sandboxed `GET /api/files`** (`api/routers/files.py`, off-gate,
     any-authenticated-role) lists one directory level under an ALLOWLISTED root
     (`PIPEGUARD_BROWSE_ROOTS`, default `{"data": <repo>/data}`) — metadata only, traversal-hardened
     like the existing artifact-download idiom (`..`/absolute → 400, an escaping symlink → 403, an
     unknown root/missing dir → 404) — powering a new `FileBrowser.tsx` picker wired into the
     custom-script inspector's locator fields. Plus `tests/test_io_path_wiring.py` (9, NEW), offline
     coverage proving the PRE-EXISTING `POST /api/pipelines/run` input picker actually flows a picked
     key through to the real driver/`nextflow run` argv (not new behavior, new proof). **Honest
     deferral:** the custom script itself runs with NO PipeGuard-side runtime sandbox — safety is the
     W1 approval gate + the honest label + deployment-side sandboxing (an ADR-0020 Assumption), not a
     sandbox PipeGuard builds. Verified: 585 tests collected / 44 files (was 555/41), 578 passed / 7
     skipped offline, ruff+mypy+tsc+oxlint clean.
...
| Node-authoring agent (advisory) | `PIPEGUARD_NODE_AUTHOR_AGENT=stub\|claude` (`PIPEGUARD_NODE_AUTHOR_MODEL`, default Sonnet); retrieves a `NodeProposal` over a curated **10-card** tool corpus (`src/pipeguard/node_author/`, T-046; was 11 until Branch A of the custom-script-card effort, 2026-07-11, retired the unwired Truth VCF reference-node card). **A read-only `GET /api/builder/node-proposal` endpoint + Builder-modal wiring shipped 2026-07-11 (W2, T-127)** — closes the earlier "core-only, reachable only by direct Python import" gap. **`POST /api/builder/node-proposal/accept` → the library store shipped the same day (T-135)** — backend-only; the Builder's own "Accept to library" button stays deferred, see [agent-authoring-contract.md](agent-authoring-contract.md) | stub ($0) |
| Sandboxed server-side file browser (off-gate, read) | `GET /api/files?root=&path=` (`api/routers/files.py`, 2026-07-11, ADR-0020's Branch B); lists one directory level under an ALLOWLISTED root (`PIPEGUARD_BROWSE_ROOTS`, a `key=abs_path` map, default `{"data": <repo>/data}`) — metadata only (name/size/ext-inferred kind), never file bytes; traversal-hardened (`..`/absolute → 400, an escaping symlink → 403, unknown root/missing dir → 404) like the existing artifact-download idiom. Powers the Builder's `FileBrowser.tsx` "Browse…" picker (additive — the manual path input stays); any authenticated role may browse | allowlisted `data/` only |
| Tool-card library store (off-gate product) | `PIPEGUARD_LIBRARY_STORE=jsonl\|sqlite` (`api/library_store.py`, 2026-07-11, T-135); holds accepted `NodeProposal`s as versioned `draft` `LibraryEntry` records, gated through `check_conformance()` at accept time — degrade-to-JSONL. **No Postgres adapter, by design**: node-local corpus of accepted drafts, not shared product state (ADR-0016 item 9), same shape as the job store | JSONL |
...
| Pipeline codegen (compose, never execute) | `pipeguard.nextflow.compile_graph()` — a card graph → Nextflow bundle; `POST /api/pipelines/compile` (JSON/`.zip`); curated catalog, uncatalogued tool → a labelled placeholder ([design/nextflow-codegen.md](nextflow-codegen.md), T-123). **Operator-authored custom-script processes ([ADR-0020](../adr/ADR-0020-operator-authored-custom-processes.md), 2026-07-11)** are a THIRD path alongside catalogued/placeholder: a node carrying a non-empty `NfNode.script` renders that verbatim body as a real, honestly-labelled process (catalog never consulted); a blank script is a `CompileError`, never fabricated; reaches a compute host only behind the pre-existing W1 `POST /api/pipelines/run` approval gate | pure text codegen, no execution |
