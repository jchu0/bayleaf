...
- **(B) BUILD NOW (MVP)** — the hackathon slice. `api/` + `frontend/` only; **zero `src/bayleaf/` change**; zero token spend; zero guardrail risk.
...
6. **Output layout + archive (D7/D8).** Keep the `fastq/` `mosdepth/` `run/` names.
   **BAM→CRAM for archiving is CONFIRMED** (overrides the earlier "defer"). A run's **archive**
   ...
   (software versions + params + execution trace), the MultiQC report, **bayleaf's own
   ...
   CRAM later**. A **config file maps artifact-kind → output path** so bayleaf
...
7. **Variant-gate substrate (General + D8/D13).** Add a **real disease-gene panel** over GIAB
   HG002 (truth-backed) + a **simple, pluggable caller** — bayleaf mostly *reads* the VCFs a
...
→ text/csv | application/x-ndjson
  Content-Disposition: attachment; filename="bayleaf-export-<scope>-<ts>.<ext>"
```
...
- **De-id gate on intake identity (G-DEID).** Persisting/exporting `subject_id`/`tissue` cohort keys is **restricted to public/synthetic/contrived origins until de-identification lands** (wishlist, not built). For the hackathon this is a non-issue in practice: the only "real" data is **GIAB HG002, a publicly consented reference genome — not PHI**; the gate is a forward-looking guardrail for when real *patient* runs are ingested.
  - **Frontend precursor, honors the same gate by staying entirely client-side (2026-07-10, Wave
    9, T-117):** a new `/accession` screen (`frontend/src/screens/Accession.tsx`) composes
    exactly the `subject_id`/`tissue`/collection-metadata fields item 3 below (§2.2(C)) would
    eventually persist server-side — but makes **zero `api` calls**; `POST /api/runs`'s
    `SubmitRunIn`/`SampleIn` still carry no subject field and reject one (`extra="forbid"`), so
    there is no wire path into the ledger yet. DOB/MRN are deliberately not modeled as fields at
    all (PHI). This is the UI half of "widen `sample.registered`" arriving *before* its backend
    persistence slice — G-DEID still holds because nothing crosses the network boundary; see
    [functional.md REQ-F-082](../requirements/functional.md) and
    [nonfunctional.md REQ-NF-023](../requirements/nonfunctional.md).
  - **Built (T-040 / W14): a config-driven de-id *policy* realizes G-PII + G-DEID at the export seam** ([`api/deid.py`](../../api/deid.py); routed through `_decision_rows`/`_feature_rows` in [`api/main.py`](../../api/main.py)). It is a **demo de-id SEAM, NOT HIPAA de-identification** — hashing is salted SHA-256 *pseudonymization*, a non-reversible heuristic, not a Safe-Harbor / Expert-Determination scrub. Four explicit field classes applied per row against the read-only `origin`: **`DROP`** (`submitted_by` operator PII — never a column, proving G-PII is policy-driven, not incidental omission); **`GATE_BY_ORIGIN`** (`subject_id`/`tissue` cohort keys — withheld for guarded origins `real-giab`+untagged `unknown`, and **pseudonymized** for non-real origins, so the opt-in `GET /api/export?include=identity` mode *shows* origin-gated hashed cohort keys rather than raw); **`HASH`** (always-pseudonymize); **`PASSTHROUGH`** (default for unnamed operational columns → a non-identity export is byte-identical to before). The default (no `include`) export is unchanged. Salt via `BAYLEAF_DEID_SALT` (documented non-secret demo default). Policy id rides the `X-Bayleaf-Deid-Policy` header (no compliance claim). **Non-laundering (§5e):** `origin` is read-only from the per-run marker and the guarded-origin set is a fixed classification — **not** env-configurable — so config can never relabel a run *up* to `real-giab`. The policy is an export-path transform only; it never touches a verdict, finding, confidence, or gate input ([ADR-0001](../adr/ADR-0001-deterministic-gate-advisory-ai.md)).
  - **Built (2026-07-10, ADR-0018 D3): a second, more conservative Safe-Harbor-***style*** scrub** — [`api/safe_harbor.py`](../../api/safe_harbor.py) — the maintainer's chosen most-conservative default: drops direct-identifier fields outright, generalizes date fields to year, caps ages > 89 at `"90+"`, and mechanically regex-redacts the §164.514(b)(2) free-text identifier classes (email/phone/SSN/URL/IP/date/ZIP/long-numeric-id) — `HIPAA_SAFE_HARBOR_CLASSES` documents all 18. **Still explicitly NOT a certified/attested de-identification** (the policy id says "style," never "HIPAA-compliant"; no Expert Determination, audit, BAA/DUA) — still wishlist #14 for the full module (free-text NLP redaction, date-shift/k-anonymity, DUA/BAA, a full audit trail beyond the run-scoped provenance event below). **Wired to a real egress (2026-07-11):** `POST /api/runs/{run_id}/share` (`require_role("approver")`) now runs it as the default (and only) policy, returns a `ShareBundle` + `ShareManifest`, and records the egress as a `DATA_EXPORTED` `ProvenanceEvent` via `api/share_store.py`, a pluggable sink distinct from the gate's own `EventLedger` — **shipped JSONL-only at first, then (later the same day, commit `9a4ef5f`) brought to the same pluggable jsonl/sqlite/postgres shape as the other four off-gate stores** (`BAYLEAF_SHARE_STORE=jsonl|sqlite|postgres`, degrade-to-JSONL on any DB failure, verified against a live `postgres:16`; [ADR-0016](../adr/ADR-0016-postgres-port.md) item 7). This is **narrower** than the full report/share surface [design/variant-interpretation.md](variant-interpretation.md) §4 describes — one fixed action, no scope/location/security-level selection, and the event lands in the run's own Provenance trail rather than the Admin Activity feed. `GET /api/export` still runs the separate, less-strict `deid.py` policy above (unchanged).
...
**Key files:** `src/bayleaf/engine.py:146` (emit `metric.parsed`), `src/bayleaf/provenance.py` (event already reserved), `src/bayleaf/persistence/{records,projector,sqlite,repository}.py` (new table + filters), `src/bayleaf/models.py:331` (`MetricValue`, reused), `api/main.py:72-116` (BUILD-NOW export endpoint; TARGET-STATE repo read).
...
inputs. Appendix C's tool-output catalog and its "bayleaf runs no alignment and no
...
*All work lands in `api/` + `frontend/`; `src/bayleaf/` untouched. Baseline: the only browse surface is `RunOverview` at `/`, one eager unparameterized `api.runs()` into a flat card grid; `Monitoring`/`ReviewQueue`/`Intake` go further and `Promise.all` full `RunDetail` for every run on mount.*
...
A **different, smaller** scale fix than §4.4/§4.5 above, landed the same day (commit `e5d5043`): with the GIAB seed there are 50+ recurring issue signatures on `GET /api/monitoring`, and `Monitoring.tsx` rendered the entire (already server-unbounded — the frontend never sends `signatures_limit`) list. Ports the Runs-list pagination pattern verbatim: a 25/50/100 `SegmentedControl` + ‹/numbered/› pager + "Showing X–Y of N signatures," client-side over the fetched array, resetting to page 1 on window/search/per-page change. **This is not the same gap as [tasks T-072](../planning/tasks.md)**, which was (at the time this section was written) the still-open, un-paginated per-run `rows: list[MonitoringRunRow]` on the same `GET /api/monitoring` response — a different list, still uncapped server-side. **T-072's frontend half is now closed — see §4.8 below.**

### 4.7 (Shipped 2026-07-10, T-093) — Admin Activity-log, client-side pagination + expandable rows

A **third instance** of the same scale-kit pagination pattern (§4.5/§4.6), applied to the `/admin` Activity-log's merged threshold/pipeline/ticket audit feed (commit `8a14661`): the flat list rendered every row uncapped — a formatting mess as the feed grows. Ports the pattern again: 25/50/100 `SegmentedControl` + ‹/numbered/› pager + "Showing X–Y of Z," client-side over the already-fetched merged array, resetting to page 1 on filter change. Additionally, each row collapses to a compact summary that **expands on click** to a labelled Detail/Target/Actor/When panel (one open at a time) — a UI affordance the Runs/Monitoring instances don't need (their rows are already single-line). No backend change; the feed is still the zero-new-backend merge of `GET /api/settings/thresholds` + `GET /api/pipelines` + `GET /api/review/tickets` described in [architecture.md](architecture.md) §Component map (Admin bullet).

### 4.8 (Shipped 2026-07-10, T-072 frontend half) — Monitoring per-run pagination, closing the §4.6 note's open gap

A **fourth instance** of the same scale-kit pattern (§4.5–§4.7), landed commit `34bca5d`: the Monitoring throughput card's per-run columns (`GET /api/monitoring`'s `runs: list[MonitoringRunRow]` — a *different* array from §4.6's recurring-signatures list, on the same response) rendered every kept run in the window uncapped. Ports the pattern again: 25/50/100 `SegmentedControl` + ‹/numbered/› pager + "Showing X–Y of N runs," client-side over the already-fetched, date-filtered array, with its own state independent of the signatures pager and a reset-to-page-1 effect on window/date-range/per-page change; `maxSamples` stays computed over the *full* filtered set so the y-axis is stable across pages. **This closes only the frontend half of [tasks T-072](../planning/tasks.md)** — the payload itself (`runs[]`) is still returned uncapped by `get_monitoring` in `api/main.py`; the row's backend cap (a real concern only once `synthetic/scale.py`, T-050, seeds a much larger window) remains the still-open reason T-072 stays `todo` in [tasks.md](../planning/tasks.md). Verified live: "Showing 1–15 of 15 runs" (30d window), independent of the signatures pager's "1–25 of 51 signatures." **§4.8's pager is superseded by §4.9 below (2026-07-10, same day) — read this section as a historical record of what shipped, not the current UI.**

### 4.9 (Shipped 2026-07-10, commit `f8a6f35`, T-100) — Monitoring throughput chart replaced with Recharts; §4.8's pager REVERSED

The maintainer's very next feedback round replaced the whole §4.5–§4.8 hand-rolled bar chart with a **Recharts** `ComposedChart` (`recharts@3.9.2`, MIT — the frontend's first real charting dependency, justified per [CLAUDE.md](../../CLAUDE.md)'s Dependencies guardrail: hover tooltips + a trend line + a stable frame aren't practical to hand-roll without reinventing a chart library). The new chart is FROZEN to a ~14-day column frame (constant per-column slot, `maxBarSize`-capped) and scrolls sideways beyond it, rather than resizing the card on a 7d/14d/30d toggle — and **this removes §4.8's per-run pager entirely**: the maintainer's own framing was "it scrolls now, so paging the throughput columns made no sense." The signatures pager (§4.6) is unaffected — only the per-run throughput columns changed.
...
`page`/`limit`/header contract exactly (`X-Bayleaf-{Total-Count,Page,Limit}`, no params → the
...
> **Built** as [`api/archivist.py`](../../api/archivist.py) (mirroring the feedback agent), realizing this design. Stub-first / $0 offline (`BAYLEAF_ARCHIVIST_AGENT=stub|claude`, Haiku tier); advisory, off the gate. It takes a least-privilege `RunArchiveInput` (no `subject_id`/`tissue`/`submitted_by`) built from the projection and emits an `ArchiveDigest` (digest + export manifest + cross-run index); it never opens/moves/deletes a file or relabels an origin, and carries no verdict. Endpoints: `GET /api/runs/{id}/archive-digest` + `GET /api/archive/index`. **Frontend wired 2026-07-10 (T-069, commit `adfd7aa`):** the Pipeline Builder's `ArchivistModal` (previously a static preview) now calls `GET /api/archive/index` and renders the real `ArchiveDigest` — n_runs/archive-ready counts, held-from-archival, origins verbatim, size, recurring signatures, proposed action, and the disclaimer below, all rendered from the endpoint's own response; "Queue archive" stays an inert button (still no write endpoint). The findability/registry pieces in §3.5 remain deferred; the sections below are the built agent's rationale + guardrail proof.

*Specced strictly within the existing agent model — advisory, off-path, off-by-default, stub-first — mirroring the QC-triage agent (`src/bayleaf/triage/`). See the standalone roster block for the drop-in version; this section carries the rationale and the guardrail proof.*
...
MODEL TIER: cheapest (Haiku), env `BAYLEAF_ARCHIVIST_MODEL`; off by default behind `BAYLEAF_ARCHIVIST_AGENT=stub|claude` (default stub) — an organizing task, not diagnosis (ADR-0012).
...
SCHEDULE NOTE: with the Archivist cut and QC-triage done, the highest-value use of the "agent layer" slot is flipping the existing QC-triage agent live for the demo (`BAYLEAF_TRIAGE_AGENT=claude`, stub fallback intact) — not starting agent #2 or #3.
...
The end-to-end artifacts a real germline panel run emits, stage by stage. **This catalog was fact-checked 2026-07-08 against `gate_giab.py`, which consumes pre-aligned/pre-called GIAB inputs and re-derives only three QC stages — bayleaf ran no alignment/calling of its own at that point.** That is now only half true: `scripts/run_giab_pipeline.py` (2026-07-09, §3.2.1) *does* run its own bwa-mem2 alignment, samtools `markdup` dedup, and bcftools variant calling, outside the app (compose ≠ execute holds — the deterministic core `src/bayleaf/` still never runs a tool). **Update (2026-07-11):** that driver was originally a standalone bioconda-toolchain script; it is now **Nextflow-first** — it hands the whole chain to `nextflow run pipelines/germline/main.nf` (a committed pipeline a new `src/bayleaf/nextflow/` compiler generates from a Builder card graph) rather than invoking `bwa-mem2`/`samtools`/`bcftools` itself; see [design/nextflow-codegen.md](nextflow-codegen.md). The tools that run and the numbers they produce are unchanged — only the orchestrator is. Rows 3/4/6 below are marked ★ where this changes their demo status. Filenames shown as `HG002.*` are quoted verbatim from our scripts (no tag); generic `<sample>.*` names are the tool/sarek convention; `(VERIFY)` marks a filename I could not confirm against the repo (version-dependent or general knowledge).
...
| 10 | **Pipeline provenance** | Nextflow / nf-core | `pipeline_info/`: `software_versions.yml`, `params_<ts>.json`, `execution_trace_<ts>.txt`, `execution_report_<ts>.html`, `execution_timeline_<ts>.html`, `pipeline_dag_<ts>.{html,svg,mmd}` (VERIFY), `manifest_<ts>.bco.json`; `csv/` recaps `mapped.csv`, `markduplicates.csv`, `recalibrated.csv`, `variantcalled.csv` (cols `patient,sample,…,path`) | Record exact tool versions, resolved params, per-task hashes/runtimes, BioCompute Object | ★ **Updated 2026-07-11 — no longer "we run no nextflow."** `scripts/run_giab_pipeline.py` now DOES run `nextflow run pipelines/germline/main.nf` for real ([design/nextflow-codegen.md](nextflow-codegen.md)), so Nextflow itself executes and *could* emit `pipeline_info/`. **Still PARTIAL, for a different reason:** nothing in this repo requests or parses that manifest — the driver hand-parses each process's own published QC file (`fastp.json`, `mosdepth` summary/thresholds) rather than Nextflow's `pipeline_info/`, and bayleaf keeps its **own** provenance — append-only JSONL event ledger + rebuildable SqliteRepository (ADR-0002), not `pipeline_info/` |
...
2. **Layer B — ACMG-SF framing.** Adds legibility ("we cover the actionable disease genes ACMG recommends reporting"). Grade only the portion inside a truth BED. The **per-gene callable fraction is itself an honest QC signal**, consistent with bayleaf's breadth-not-just-depth doctrine.
...
**2a. Contract — bayleaf is the reader, not the caller.** A variant caller (DeepVariant, GATK HaplotypeCaller, bcftools, FreeBayes) runs inside *someone's* Nextflow pipeline and drops a standard VCF into a run directory. bayleaf points at that folder and ingests whatever VCF is there. The caller is **BYO**; bayleaf owns only the read + gate (maintainer's verbatim intent).
...
- **(BYO, higher fidelity — CUT to a doc sentence)** DeepVariant is the drop-in upgrade a user brings (better SNV/indel accuracy; emits `GQ`+`VAF` directly). It changes nothing about bayleaf: same folder, same standard VCF. Name it in docs; do not build it.
...
**6c. Do not headline the most alarming actionable-cancer genes.** Do **not** headline *BRCA1/2* or *TP53* as demo fixtures unless HG002 is **confirmed** to carry a qualifying P/LP allele there (§6d) — "bayleaf found a pathogenic BRCA1 variant in this genome" is the single highest misread risk.
...
GROUND 1 §4 states no per-variant metrics are defined; **this is now stale.** Verified 2026-07-08: `src/bayleaf/metrics/metric_registry.yaml` declares `variant.dp`, `variant.gq`, `variant.allele_balance` (parser `vcf_allele_balance`), `variant.titv` (source `bcftools_stats`), each `gate: variant`. They are **inert vocabulary** — no locator (§5), no parser wired, no rule in `rules.py` consuming them (grep confirms zero `Category.VARIANT` findings fire). Leaving them declared-but-inert is correct; *finishing* them (locator + parser + aggregation rule) is Phase 2.
...
| 10 | Run-layout **loader + `load_run` refactor + `BAYLEAF_RUN_LAYOUT`** | D3 §5 | **DEFER** | Refactors the one tested path for zero demo gain; pydantic-settings first use in `src/` |
...
- Core seams for Phase 2: `/Users/jchu/IdeaProjects/claude_life_science_hackathon/src/bayleaf/models.py` (`Gate.VARIANT`/`Category.VARIANT` defined), `.../src/bayleaf/rules.py` (no VARIANT findings yet), `.../src/bayleaf/metrics/metric_registry.yaml` (`variant.*` keys declared, lines ~290-352, no locator/parser/rule), `.../src/bayleaf/parsers.py:187` (hard-coded five-file `load_run`)
- Proposed new committed artifacts: `scripts/panel_regions.disease.bed` (+ provenance header), a fixtures manifest, `src/bayleaf/layout/run_layout.yaml` (schema only, DESIGN-NOW)
...
5. **pydantic-settings first use in `src/`.** The run-layout `BAYLEAF_RUN_LAYOUT` selector would be the **first** pydantic-settings use in `src/` (CLAUDE.md coding-standard 4 names it as intended but it is unused today). Introducing a new config mechanism in crunch week is a poor bet — recommendation is DEFER the loader/refactor and ship only the schema in the doc. Confirm, or approve introducing pydantic-settings now.
