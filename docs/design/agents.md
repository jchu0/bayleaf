...
|---|---|
| **Status** | Active |
| **Last updated** | 2026-07-13 (MST) — first end-to-end LIVE smoke-test of all six agent seams over the API, which surfaced two bugs both now fixed + regression-tested: the QC-triage `ask` live-path truncation (its own `_ASK_MAX_TOKENS` budget + stop-reason guard) and the Archivist 500 on a clean run (eager `signatures[0]`); reconciled the taxonomy section to the shipped state: **System Agents is now its own page/`PageId`** (`/system-agents`, dedicated `SystemAgents.tsx`, grounded in `App.tsx`/`access.ts`/`Sidebar.tsx`); page-name simplification "Agent triage" → **"Triage"**; the System-agents/Agent-triage IA split + the `ask` endpoint's `require_role` floor (QC-triage roster row) |
...
|---|---|---|---|---|---|
| 1 | **QC-triage** | On a flagged card, suggest a likely cause + next action, cited | **done** (T-015); interactive **`ask`** sibling **done** (WS-07 Q2, T-144) — `POST /api/runs/{run_id}/cards/{sample_id}/ask` answers a free-text question about any card, even a clean PROCEED one, `advisory: Literal[True]`, no verdict/confidence. **Authz seam closed (2026-07-13, T-164, commit `a1aef73`):** this was the only advisory endpoint with no `require_role` dependency at all; now gated `viewer`+ (the read-family floor — advisory, not a mutation, so it does not take the reviewer+ write floor). **Live-path fix (2026-07-13):** the free-text `ask` answer now uses its own larger token budget (`_ASK_MAX_TOKENS`, distinct from `triage_card`'s two-field note) + an explicit `max_tokens`/`refusal` stop-reason guard — before this a long grounded answer on a *flagged* card truncated mid-JSON and silently degraded to the stub (proven live over the API, first end-to-end agent smoke-test, see [journal](../journal/2026-07-13-live-agent-api-smoke-test.md)). **Note cache (2026-07-13):** `GET .../triage` is cache-through (`api/triage_cache.py` + `api/triage_cache_store.py`) — a note is generated once per stable card-signature+agent key and served from the backend on repeat requests (no regeneration / Claude re-call on navigation), saved + logged as a structured record; a transient live→stub degrade is not cached (retries). See [journal](../journal/2026-07-13-triage-note-cache.md) | [`triage/`](../../src/bayleaf/triage/), [ADR-0012](../adr/ADR-0012-agent-scoping-model-tiering.md) | mid |
...
| 3 | **Archivist** | Index / organize / summarize / prepare exports across runs (the "librarian" over the data platform) | **done** (T-059) — advisory, **off the gate**; indexes/summarizes already-decided runs → `ArchiveDigest` (digest + export manifest + cross-run index); organizational not diagnostic; least-privilege input (no PII); never opens/moves/deletes a file or relabels an origin. **Bug fixed (2026-07-13):** the deterministic `_summary_prose` indexed `signatures[0]` eagerly, so `GET /api/runs/{id}/archive-digest` **500'd for any clean, all-PROCEED run** (no recurring signatures) — found in the first live smoke-test, now guarded + regression-tested (see [journal](../journal/2026-07-13-live-agent-api-smoke-test.md)) | [`api/archivist.py`](../../api/archivist.py), [data-platform-and-archivist.md](data-platform-and-archivist.md) §5 | low / Haiku (organizational, not diagnostic) |
