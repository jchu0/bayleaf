...
5. **Dashboard — 9 operator screens, rebuilt to the refreshed design prototype** (T-062,
   2026-07-09, superseding T-022b's 1:1 migration): submit samplesheet, run overview, intake/
   preflight, decision cards, review queue, provenance (compute-DAG), agent triage, monitoring,
   settings — in a two-group nav (Operate/Configure) — **plus the Pipeline Builder** (T-044/#11,
   with free composition + Connect mode since T-062); plus the Streamlit offline fallback.
