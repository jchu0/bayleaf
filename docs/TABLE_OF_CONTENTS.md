...
|---|---|
| **Status** | Active |
| **Last updated** | 2026-07-12 (MST) — ADR-0022 agent-observation-binding (Builder agent-attachment is a persisted, read-only `AgentBinding` off the compiled graph; a scoped, de-identified node-read path; pipeline-vs-system agent taxonomy); `bayleaf` rebrand (product renamed; the Python package/`PIPEGUARD_*` env vars stay `pipeguard`); ADR-0021 operator-gated/scheduled processing; ADR-0020 operator-authored custom-script processes + sandboxed `GET /api/files` browse + compiler robustness hardening; node-author corpus **9** (NGSCheckMate retired-but-pinned); dated build chronology relocated to [HISTORY.md](HISTORY.md) |
