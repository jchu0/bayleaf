...
| [design/run-layout-emit-consume.md](design/run-layout-emit-consume.md) | 🚧 | **Scoping (Proposed)** — the full Pipeline-Builder `run_layout.yaml` **emit → consume** loop: a current-state audit (a UI-complete but graph-**decoupled** emit shell; `load_run` still hardcodes the frozen-five filenames; two un-unified locate mechanisms), the gap (G1–G7), the invariants, a **4-phase** build plan, and **2 open decisions** (`pydantic-settings` vs `os.getenv`; unify-now vs parallel). Distinct from the built Export-to-Nextflow codegen ([nextflow-codegen.md](design/nextflow-codegen.md)) |
...
| [ADR-0024](adr/ADR-0024-scope-by-wiring.md) | **Accepted — enforcement shipped (P2, 2026-07-13).** An agent's file access = the tools it is **wired to** (server-enforced, advancing ADR-0022 from advisory hint to enforced): a run's executed-graph bindings are snapshotted at launch — Builder-Run first, **intake-authored runs too as of T-148 (PR #10, same day)** — and `GET .../nodes/{node}/observations?agent=X` 403s unless X is wired to the node, capping grants to the binding. Access ≠ authority (ADR-0001). Settings "Operator profile" bar removed (§6). **Deferred:** the auto agent-consumption `agent` pass + a real triage consumer. |
