...
| [ADR-0016](adr/ADR-0016-postgres-port.md) | Postgres port (guarded, off-by-default): the `Repository` Postgres adapter + five pluggable jsonl/sqlite/postgres off-gate stores (feedback/pipeline/review/settings/share) + a sixth, jsonl/sqlite-only durable job store (`api/job_store.py`, no Postgres adapter by design — node-local scratch) |
