...
| [design/agent-capabilities.md](design/agent-capabilities.md) | 🚧 | Per-agent grounding/authoring upgrades. **§1 archivist read-only cross-run DB retrieval BUILT** (P3, 2026-07-13, `api/archivist_retrieval.py`; projection-first + run-dir-derivation fallback; grounds the archivist chat; live-verified). Still design: §2 pipeline-repair issues+resolutions + tool/bayleaf docs corpora (issues half needs the monitoring tool-agent); §3 node-author QoL popup + doc-upload + schema-delta-as-proposal + scaffolds-as-assets. All advisory, structured-for-ML. |
