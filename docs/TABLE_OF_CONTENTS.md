...
| [design/agent-capabilities.md](design/agent-capabilities.md) | 🚧 | Per-agent grounding/authoring upgrades. **§1 archivist read-only cross-run DB retrieval BUILT** (P3, 2026-07-13, `api/archivist_retrieval.py`; projection-first + run-dir-derivation fallback; grounds the archivist chat; live-verified). **§2 pipeline-repair docs corpus BUILT** (P3, 2026-07-13, `pipeline_repair/knowledge/bayleaf_system.jsonl` + catalog-derived tool docs via `load_system_corpus`; grounds the repair chat; live-verified) — its **issues+resolutions half stays deferred** (needs the monitoring tool-agent). **§3 node-author Accept-to-library button BUILT** (P3, 2026-07-13, `AuthorToolNodeModal` → `POST .../node-proposal/accept`, confirm+role-gated, live-verified). Still design: §3 doc-upload → schema-delta-as-proposal + scaffolds-as-assets; §2 issues+resolutions half. All advisory, structured-for-ML. |
