...
| [design/system-agents-chat.md](design/system-agents-chat.md) | 🚧 | **Proposed** — the System agents page redesigned as a **chat surface** (left agent-select panel + chat window over pipeline-repair/archivist) with **structured, ML-minable chat history** (`chat_store`; view-scoped archive/delete = soft-delete, records retained). Reuses the `ask` plumbing; advisory only (ADR-0001). |
| [design/agent-capabilities.md](design/agent-capabilities.md) | 🚧 | **Proposed** — per-agent grounding/authoring upgrades: archivist **read-only DB retrieval** (de-id, cited); pipeline-repair **issues+resolutions store + tool/bayleaf docs corpora**; node-author **authoring popup (QoL) + doc-upload + schema-delta-as-proposal + scaffolds-as-assets** (generalized to all agents). All advisory, structured-for-ML, versioned. |
...
| [ADR-0024](adr/ADR-0024-scope-by-wiring.md) | **Proposed** — an agent's file access = the **union of the output folders of the tools it is wired to** (server-enforced, advancing ADR-0022 from advisory hint to enforced), resolved from the **operator-configured data root** (not repo-local `data/`), reusing the `GET /api/files` sandbox + de-id. Retires settings-based agent scoping; consolidates the view-only "Operator profile" bar into the persisted "Card density" pref. Access ≠ authority (ADR-0001). |
