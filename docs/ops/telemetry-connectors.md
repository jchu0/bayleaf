...
How to point an APM (Datadog, Prometheus, or any OTLP backend) at PipeGuard's
shipped `GET /metrics` endpoint. This is **config + docs only** — no PipeGuard
code and no new dependency. Everything here rides the pull model: your collector
scrapes `/metrics`; PipeGuard never dials out and never ships a credential.
...
2. **Fact.** Collectors GET that endpoint on their own schedule. PipeGuard makes
   no outbound network call and holds no APM credentials — the credentials live in
   the collector/agent config, never in PipeGuard.
...
|---|---|---|---|
| `pipeguard_runs_total` | counter | — | Analysis runs discoverable by the API |
| `pipeguard_samples_total` | counter | — | Decision cards (samples) across served runs |
| `pipeguard_cards_total` | counter | `verdict` ∈ {`proceed`,`hold`,`rerun`,`escalate`} | Decision cards by final verdict |
| `pipeguard_gate_flagged_samples_total` | counter | `gate` ∈ {`preflight`,`qc`,`variant`} | Samples with a non-proceed verdict at each gate |
...
> Guardrail: PipeGuard is a research/demo tool, not a clinical system. These are
> aggregate operational counts, not calibrated or clinical metrics.

## Connect Datadog

Use the Datadog Agent's OpenMetrics V2 check — no PipeGuard change.
...
3. Restart the Agent. Series arrive namespaced `pipeguard.*`; under OpenMetrics V2
   a `_total` counter lands as `pipeguard.<name>.count`
   (e.g. `pipeguard.cards.count`, keeping the `verdict`/`gate` tag dimensions).
...
3. Reload Prometheus; confirm the `pipeguard` target is **UP** on the Targets page.
...
`opentelemetry-*` + OTLP) behind a `PIPEGUARD_*_LIVE` opt-in, following the
