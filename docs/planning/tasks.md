...
| T-037 | Provenance stage/swimlane canvas: reshape the gate event trail into a staged view (run→samples→findings→verdict→completed) with per-node inputs/outputs + per-sample metric_values drill-down; frontend-only over GET /api/runs/{id}; defer real sarek compute-DAG | Phase 2 (frontend) | in-progress | yes | none |
