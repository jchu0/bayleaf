...
| T-045 | Frontend fidelity + scale backlog — from a 6-agent fidelity/scale audit (2026-07-09) synthesized into a claude-design handoff ([`design/frontend/handoffs/2026-07-09-review-to-design.md`](../design/frontend/handoffs/2026-07-09-review-to-design.md)). Buckets: **fix-now** (Decision-cards density lever/left-stripe/chips/evidence; Pipeline-Builder port edges + wired export/params/validation coloring; Runs skeletons/Retry/status-pill; Settings profile toggle + editable steppers + honest copy; Monitoring repair-agent button + window labels), **phase-2** (a shared scale kit — search/facet/paginate/virtualize/density — + synthetic volume + windowed backend aggregates + `RunSummary` status/date/platform; the Decision-card QC metric readout via `MetricsPanel`; Pipeline-Builder editable locators→live YAML + save/version/approval; per-agent model tiering), **design-question** (scale interaction per surface; threshold-edit RBAC/audit; the #11 node-authoring agent) | Port | todo | yes | T-022b, T-044 |
