...
| T-029 | NGS analysis-output **layout** convention — expand data-platform §3: a full tool-output catalog (fastq · BAM→CRAM · VCF/gVCF · coverage · QC · reports · pipeline_info) + coherent grouping-by-artifact-class for findability + storage strategy. Additive; the gate's flat `run/` five-CSV ingest stays frozen | 2 | in-progress | no | T-017 |
| T-030 | BUILD-NOW data slice (pending maintainer go) — read-only `GET /api/export` over the in-memory cards (`grain=decision` CSV + `grain=feature` JSONL = the ML corpus) + month-scoped `RunsBrowser` + download button. `api/`+`frontend/` only; zero core change | Port | todo | no | T-028 |
