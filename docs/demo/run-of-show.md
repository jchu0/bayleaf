...
|---|---|---|---|---|---|
| 1 | 0:20 | **Hook** — Run Overview (`/`) | Browser already on the run list; point at `mock_run_01`'s "needs attention" flag | "A run just finished. Someone has to decide, per sample, proceed / hold / rerun / escalate — today that's combing logs by hand." | If list empty → reload; if still empty → reseed (`scripts/seed_giab_demo.py`) or cut to the recorded walkthrough |
