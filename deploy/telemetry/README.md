...
Vendor-neutral, **pull-based** telemetry connectors that wire an APM to bayleaf's
already-shipped `GET /metrics` endpoint. Config + docs only — no bayleaf code or
...
| `grafana-dashboards.yml` + `grafana-dashboard.json` | Grafana | Auto-provisioned **"bayleaf — QC decision gate"** board over the four `/metrics` series (runs, samples, cards-by-verdict, flagged-by-gate) |
